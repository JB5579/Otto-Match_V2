...
class PipelineValidator:
    ...
    async def validate_pipeline(self, pdf_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"\n🔄 Step 2: Processing PDF and persisting to database...")
            process_result = await self.pdf_service.process_and_persist_pdf(
                pdf_bytes=pdf_bytes,
                filename=Path(pdf_path).name
            )

            print(f"   ✅ Listing ID: {process_result['listing_id']}")
            print(f"   ✅ VIN: {process_result['vin']}")
            print(f"   ✅ Images: {process_result['image_count']}")
            print(f"   ✅ Issues: {process_result['issue_count']}")
            ...
            print(f"\n🔍 Step 3: Verifying listing in database...")
            db_listing = await self.listing_repo.get_by_id(listing_id)

            if db_listing:
                print(f"   ✅ Found in vehicle_listings table")
            ...
            print(f"   ✅ Found {len(db_images)} images in vehicle_images table")
            ...
            print(f"\n🌐 Step 4: Verifying in search API...")
            ...
            print(f"\n🖼️  Step 5: Validating frontend compatibility...")
            ...
            if frontend_data.data:
                ...
                print(f"   ✅ Compatible with frontend Vehicle type")
                print(f"      Required fields present: all ✅")

                # Check images
                images = await self.image_repo.get_by_listing(listing['id'])
                print(f"   ✅ Images compatible: {len(images)} images with URLs")
                for img in images[:3]:  # Show first 3
                    url = img.get('web_url') or img.get('thumbnail_url') or img.get('original_url')
                    has_url = url is not None
                    print(f"      - {img['category']}: {'✓ ' + url[:50] if has_url else '✗ no URL'}")
            else:
...
async def main():
    ...
    if not pdf_path.exists():
        ...
        print(f"\n💡 Use --list-samples to see available sample PDFs")
        sys.exit(1)
